...
# -----------------------
def _emit_header(lines: List[str], ndm: int, ndf: int, has_rigid_diaphragms: bool = False) -> None:
    ...
    # Add constraint handler setup if rigid diaphragms are present
    if has_rigid_diaphragms:
        lines.append("    # Set constraint handler for rigid diaphragms")
        lines.append("    # CRITICAL: Must use Transformation for models with rigidDiaphragm")
...
def _emit_geom_if_needed(lines: List[str], tr: int, kind: str, emitted: Set[int],
                         joint_offset_i: Optional[List[float]] = None,
                         joint_offset_j: Optional[List[float]] = None) -> None:
    """
    Emit a geomTransf command if not already emitted.
    ...
    tr : int
        Transformation tag
    kind : str
        Element type (BEAM or COLUMN)
    emitted : Set[int]
        Set of already emitted transformation tags
    joint_offset_i, joint_offset_j : List[float], optional
        Joint offsets [dx, dy, dz] for -jntOffset parameter
    """
    if tr in emitted:
        return

    # Check if we have non-zero offsets
    has_offsets = False
    if joint_offset_i and joint_offset_j:
        # Check if any offset component is non-zero (using small tolerance)
        if any(abs(x) > 1e-12 for x in (*joint_offset_i, *joint_offset_j)):
            has_offsets = True

    if kind.upper() == "COLUMN":
        if has_offsets:
            lines.append(f"    geomTransf('Linear', {int(tr)}, 1, 0, 0, '-jntOffset', "
                        f"{joint_offset_i[0]:.9g}, {joint_offset_i[1]:.9g}, {joint_offset_i[2]:.9g}, "
                        f"{joint_offset_j[0]:.9g}, {joint_offset_j[1]:.9g}, {joint_offset_j[2]:.9g})")
        else:
            lines.append(f"    geomTransf('Linear', {int(tr)}, 1, 0, 0)")
    else:  # BEAM
        if has_offsets:
            lines.append(f"    geomTransf('Linear', {int(tr)}, 0, 0, 1, '-jntOffset', "
                        f"{joint_offset_i[0]:.9g}, {joint_offset_i[1]:.9g}, {joint_offset_i[2]:.9g}, "
                        f"{joint_offset_j[0]:.9g}, {joint_offset_j[1]:.9g}, {joint_offset_j[2]:.9g})")
        else:
...
def _emit_columns(lines: List[str], cols_json: Dict[str, Any],
    ...
    for c in cols:
        ...
        # Extract joint offsets from JSON
...
def _build_explicit(ndm: int, ndf: int,
    ...
    # Load diaphragm data first to check if rigid diaphragms exist
    d_json = _read_json(diaph_path)
    has_rigid_diaphragms = bool(d_json.get("diaphragms"))

    _emit_header_and_defs(lines, ndm, ndf, ov, has_rigid_diaphragms)
